// Package v2020 implements the FastCDC 2020 algorithm with 2-byte rolling optimization.
// See https://ieeexplore.ieee.org/document/9055082 by Wen Xia, et al.
// This is the implementation used in https://github.com/nlfiedler/fastcdc-rs
//
// mostly copied from fastcdc.go, with modifications.
package v2020
...
//
// The program to produce this table is:
...
// Chunker implements the FastCDC 2020 content defined chunking algorithm.
// This version processes 2 bytes per iteration for improved performance.
type Chunker struct {
	minSize  int
	maxSize  int
	normSize int
...
type Options struct {
	...
	// no effect on the chunking output, but performance is improved with larger buffers.
...
func NewChunker(rd io.Reader, opts Options) (*Chunker, error) {
	...
	if opts.Seed != 0 {
		...
		for i := 0; i < len(gear); i++ {
			gear[i] = gear[i] ^ opts.Seed
			gearLS[i] = gearLS[i] ^ seedLS
		}
	}
...
func (c *Chunker) nextChunk(data []byte) (int, uint64) {
	...
	// Bounds check: https://go101.org/optimizations/5-bce.html
	_ = data[remaining-1]

	// Optimization: Use direct indexing with a += 2 instead of i++ with a = i * 2
	// to avoid multiplication in the hot loop, and also pre-compute the bounds.
	minLimit := c.minSize &^ 1
	...
	for a := minLimit; a < centerLimit; a += 2 {
		hash = (hash << 2) + gearLS[data[a]]
		if (hash & c.maskSLs) == 0 {
			return a, hash
		}
		hash = hash + gear[data[a+1]]
		if (hash & c.maskS) == 0 {
			return a + 1, hash
		}
	}
...
func (opts Options) validate() error {
	...
	if opts.AverageSize > opts.MaxSize || opts.AverageSize < opts.MinSize {
		return errors.New("option AverageSize must be between MinSize and MaxSize")
	}
